Config.load.before({ "BufReadPre", "BufNewFile" }, function()
	...
	Config.plugin.add("nvim-treesitter/nvim-treesitter")
...
Config.load.before({ "BufReadPre", "BufNewFile" }, function()
	Config.plugin.add("nvim-treesitter/nvim-treesitter-textobjects")

	local move = require("nvim-treesitter-textobjects.move")
	Config.autocmd.create("FileType", function(ft)
		local function map(lhs, method, query)
			Config.keymap.set("nox", lhs, function()
				move[method](query, "textobjects")
			end, { silent = true, buffer = ft.buf })
		end
