...
local function focus(source, opts)
  return function()
    ...
    if vim.api.nvim_get_current_win() == before then ntc.execute(base_args) end
  end
end
...
local function restore_original()
  if not og.buf or not vim.api.nvim_buf_is_valid(og.buf) then return end
  ...
  if og.pos then pcall(vim.api.nvim_win_set_cursor, win, og.pos) end
end
...
map("n", fs_key, focus("filesystem", { reveal = true }))
map("n", git_key, focus("git_status"))
map("n", buffers_key, focus("buffers"))
...
local function auto_open()
  ...
  if not state or not AUTO_OPEN_SOURCES[state.name] or not state.tree then return end

  local node = state.tree:get_node()
  if not node or node.type ~= "file" or not node.path then return end
  if node.path == last_path then return end
  last_path = node.path
...
vim.api.nvim_create_autocmd("WinEnter", {
  callback = function()
    if opening then return end -- ignore auto_open bounce-backs
    if vim.bo.filetype ~= "neo-tree" then return end

    local prev = vim.fn.win_getid(vim.fn.winnr("#"))
    if prev == 0 or not vim.api.nvim_win_is_valid(prev) then return end
    local pbuf = vim.api.nvim_win_get_buf(prev)
    if vim.bo[pbuf].buftype ~= "" then return end -- only real file windows
